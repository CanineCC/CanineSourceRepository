...
{
  public record EnvironmentsUpdated(Guid ContainerId, Guid FeatureId, long FeatureRevision, Environment[] Environment);
  ...
  {
    var draftAggregate = await session.Events.AggregateStreamAsync<DraftFeatureComponentAggregate>(featureId, token: ct);
    if (draftAggregate == null) return new ValidationResponse(false, $"Draft feature '{featureId}' was not found", ResultCode.NotFound);

    var aggregate = await session.Events.AggregateStreamAsync<FeatureComponentAggregate>(featureId, token: ct);
    ...
    await session.RegisterEventsOnBpnFeature(ct, featureId, causationId, new EnvironmentsUpdated(
      ContainerId: draftAggregate.ContainerId,
      FeatureId: featureId,
