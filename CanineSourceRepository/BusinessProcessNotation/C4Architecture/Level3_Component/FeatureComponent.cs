...
public class FeatureComponentAggregate
{
  ...
  public static void Apply(FeatureComponentAggregate componentAggregate, IEvent<FeatureReleased> @event)
  {
    componentAggregate.Id = @event.StreamId;
    componentAggregate.ContainerId = @event.Data.ContainerId;
    componentAggregate.Revision = @event.Data.Revision;
    componentAggregate.Tasks = @event.Data.Tasks;
    componentAggregate.Transitions = @event.Data.Transitions;
    componentAggregate.ComponentDiagram = @event.Data.ComponentDiagram;
  }
}

public class FeatureComponentProjection : SingleStreamProjection<FeatureComponentProjection.BpnFeature>
{
  public static void RegisterBpnEventStore(WebApplication app)
  {
    ...
    {
      var bpnFeature = await session.Query<FeatureComponentProjection.BpnFeature>().Where(p => p.Id == featureId).SingleOrDefaultAsync();
  ...
  .Produces(StatusCodes.Status200OK, typeof(FeatureComponentProjection.BpnFeatureRevision))
  .WithTags("Feature");
    ...
    public static void Apply(BpnFeature projection, IEvent<FeatureReleased> @event)
    {
      ...
      if (currentNewest < @event.Data.Revision)
      {
        var newRevision = new BpnFeatureRevision()
        {
          ...
          ComponentDiagram = @event.Data.ComponentDiagram,
